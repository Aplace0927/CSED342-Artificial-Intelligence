...
def test_1_1():
    tests = [
        [
            np.array([[0, 0, -50, 20], [0, 0, -50, 0], [2, 0, 0, 0]], dtype=np.float64),
            1,
            -1,
            10,
            np.array(
                [[13, 14, -50, 20], [14, 15, -50, 19], [2, 16, 17, 18]],
                dtype=np.float64,
            ),
        ],
        [
            np.array([[0, -50, 10, 0], [0, 0, -30, 0], [0, 0, 0, 0]]),
            1,
            -1,
            10,
            np.array([[2, -50, 10, 9], [3, 4, -30, 8], [4, 5, 6, 7]], dtype=np.float64),
        ],
    ]
    for grid_world, discount, moveReward, numIters, answer in tests:
        volcano_crossing = submission.VolcanoCrossing(
            grid_world, discount=discount, moveReward=moveReward
        )
        grader.requireIsEqual(answer, volcano_crossing.value_iteration(numIters))


grader.addBasicPart(
    "1a-1-basic", test_1_1, 3, description="Basic test for VolcanoCrossing."
)
...
def test_1_2():
    tests = [
        ...
        [
            np.array([[0, -50, 10, 0], [0, 0, -30, 0], [0, 0, 0, 0]], dtype=np.float64),
            0.5,
            -0.3,
            10,
        ],
    ...
    if solution_exist:
        for grid_world, discount, moveReward, numIters in tests:
            answer = solution.VolcanoCrossing(
                grid_world, discount=discount, moveReward=moveReward
            )
            pred = submission.VolcanoCrossing(
                grid_world, discount=discount, moveReward=moveReward
            )
            grader.requireIsEqual(
                answer.value_iteration(numIters), pred.value_iteration(numIters)
            )


grader.addHiddenPart(
    "1a-2-hidden", test_1_2, 2, description="Hidden test for VolcanoCrossing."
)
...
def test_3a_1():
    ...
    rl = submission.Qlearning(
        mdp.actions, mdp.discount(), submission.identityFeatureExtractor, 0
    )
...
def test_3a_2():
    ...
    if solution_exist:
        ...
        all_states = [state for state in mdp.states if not mdp.isEnd(state)]
        grader.requireIsTrue(
            (
                sum(int(pred[state] == answer[state]) for state in all_states)
                / len(all_states)
            )
            > 0.95
        )
...
grader.addHiddenPart(
    ...
    2,
    maxSeconds=3,
    description="Hidden test for incorporateFeedback(). Run Qlearning on a small MDP, then ensure that getQ returns reasonable policy.",
)
...
grader.addHiddenPart(
    ...
    2,
    maxSeconds=3,
    description="Hidden test for incorporateFeedback(). Run SARSA on a small MDP, then ensure that getQ returns reasonable policy.",
)
...
def test_3c_1():
    ...
    rl = submission.Qlearning(
        mdp.actions, mdp.discount(), submission.blackjackFeatureExtractor, 0
    )
    # We call this here so that the stepSize will be 1
    rl.numIters = 1
...
grader.addBasicPart(
    ...
    3,
    maxSeconds=3,
    description="Basic test for blackjackFeatureExtractor.  Runs Qlearning using blackjackFeatureExtractor, then checks to see that Q-values are correct.",
)
...
def test_3c_2():
    ...
    rl = Qlearning(
        mdp.actions, mdp.discount(), submission.blackjackFeatureExtractor, 0.2
    )  # 0.2 is the default epsilon
    random.seed(SEED)
    ...
    all_states = [state for state in mdp.states if not mdp.isEnd(state)]
    agreement = sum(
        int(rl_policy[state] == vi_policy[state]) for state in all_states
    ) / len(all_states)
...
grader.addHiddenPart(
    ...
    2,
    maxSeconds=20,
    description="Hidden test for incorporateFeedback(). Run Qlearning on a large MDP, then ensure that getQ returns reasonable policy.",
)
