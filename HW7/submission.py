...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        grid = [
            [
                (
                    util.pdf(
                        math.dist([util.colToX(c), util.rowToY(r)], [agentX, agentY]),
                        Const.SONAR_STD,
                        observedDist,
                    )
                    * self.belief.getProb(r, c)
                )
        ...
        for r in range(self.belief.numRows):
            for c in range(self.belief.numCols):
                self.belief.setProb(r, c, grid[r][c])
    ...
    def elapseTime(self) -> None:
        ...
        for ((prevR, prevC), (nextR, nextC)), p in self.transProb.items():
            prob[(nextR, nextC)] += p * self.belief.getProb(prevR, prevC)

        self.belief = Belief(self.belief.numRows, self.belief.numCols, 0.0)

        for ((prevR, prevC), (nextR, nextC)), _ in self.transProb.items():
            self.belief.addProb(nextR, nextC, prob[(nextR, nextC)])
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for (r, c), n_par in self.particles.items():
            W[(r, c)] = (
                util.pdf(
                    math.dist([agentX, agentY], [util.colToX(c), util.rowToY(r)]),
                    Const.SONAR_STD,
                    observedDist,
                )
                * n_par
        ...
        for _ in range(self.NUM_PARTICLES):
            self.particles[util.weightedRandomChoice(W)] += 1
        # END_YOUR_CODE
    ...
    def elapseTime(self) -> None:
        ...
        par_next = collections.defaultdict(int)
        for par in self.particles:
            for _ in range(self.particles[par]):
                par_next[util.weightedRandomChoice(self.transProbDict[par])] += 1
        self.particles = par_next
        # END_YOUR_CODE
